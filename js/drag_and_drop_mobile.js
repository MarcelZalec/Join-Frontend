...
function updateHTML() {
  allCategories.forEach((container) => {
    ...
    if (element) {
      let filteredTasks = tasks.filter((task) => {if (task.category === container) return task});
      element.innerHTML = "";
...
async function moveTo(container) {
  ...
  if (task != undefined) {
    try {
      await saveTaskToFirebase(task, task.id);
    } catch (error) {
...
async function moveTasksToCategory(taskIndex, newCategory) {
  console.log(taskIndex, newCategory)
  let task = tasks.find((task) => task.id == taskIndex);
  console.log("Task zum verscieben:", task)
  if (task) {
    ...
    try {
      await saveTaskToFirebase(task, taskIndex);
    } catch (error) {
      console.error("Fehler beim Speichern der tasks in der Firebase-Datenbank:", error);
    }
  }

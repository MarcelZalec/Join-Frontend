...
function checkIfAssigned(contact, taskIndex) {
  task = getIndexOfTask(taskIndex)
  return task.assigned.some((assignedContact) => assignedContact.name === contact.name);
}
...
function createTaskForEditing(task) {
  let { title, description, date, priority, subtasks, assigned } = task;
  let taskForEditing = {
    newTitle: title,
    newDescription: description,
    newDate: date,
    newPriority: priority,
    newSubtaskArray: subtasks,
  };
  if (assigned) {
 ...
 * @param {string} taskId - The unique identifier of the task to be updated.
 * @param {Object} objectForEditing - An object containing the updated properties for the task. 
 * @param {string} category - The container associated with the task.
 * @param {string} type - The type of the task.
 * @returns {Object} The updated task object.
 */

function saveChangesSingleTaskWithoutSubtask(taskId, objectForEditing, category, type) {
  tasks[taskId] = {
    assigned: objectForEditing["newAssignedTo"],
    type: type,
    category: category,
...
async function updateTasksThroughEditing(taskId, objectForEditing) {
  let updatedTask = await updateTask(taskId, objectForEditing);
  checkBoxCheckedJson = {};
  return updatedTask;
}
...
async function updateTask(taskId, objectForEditing) {
  task = getIndexOfTask(taskId)
...
async function saveTaskWithCatch(task, taskId) {
  try {
    await saveTaskToFirebase(task, taskId);
  } catch (error) {
 ...
 * @param {string} taskId - The unique identifier of the task to be updated.
 * @param {Object} objectForEditing - An object containing the new details for the task.
 * @param {string} category - The identifier of the container where the task is displayed.
 * @param {string} type - The type of the task.
 * @returns {Object} The updated task object excluding subtask information.
 */

function saveChangesWithoutSubtask(taskId, objectForEditing, category, type) {
  return saveChangesSingleTaskWithoutSubtask(taskId, objectForEditing, category, type);
}
...
function filterTasksByCategory(categoryContainer, tasks) {
  return tasks.filter((task) => task.category === categoryContainer);
}

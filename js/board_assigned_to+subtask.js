...
function checkBigEditTaskContact(i, contactObject, taskIndex) {
  let task = getIndexOfTask(taskIndex)
  if (task.assigned) {
    assignedToContactsBigContainer = task.assigned;
  } else {
...
function addContactToAssigned(contactObject, taskIndex) {
  let taskJson = getIndexOfTask(taskIndex);
  let contactToUpload = {
    name: contactObject.name,
    color: contactObject.color
  }
  assignedToContactsBigContainer.push(contactToUpload);
...
function insertSubtasksIntoContainer() {
  task = getIndexOfTask(renderCurrentTaskId)
  document.getElementById("big-edit-task-subtask-container").innerHTML = "";
  ...
  } else if (subtaskArray && subtaskArray.length == 0 && task["subtasks"]) {

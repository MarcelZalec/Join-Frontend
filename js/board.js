...
function getTaskContainer(task) {
  ...
  if (task.category == "todo") {
    ...
    return task.category = "in-progress-container"
  } else if (task.category == "feedback") {
    return task.category = "await-feedback-container"
  } else if (task.category == "done"){
    return task.category = "done-container"
  }
}
...
function fixCategory(task) {
  if (task.category == "to-do-container") {
    return task.category = "todo"
  } else if (task.category == "in-progress-container") {
    return task.category = "progress"
  } else if (task.category == "await-feedback-container") {
    return task.category = "feedback"
  } else if (task.category == "done-container"){
...
function showBigTaskPopUp(id) {
  let task = getIndexOfTask(id)
...
function hideBigTaskPopUp() {
  ...
  if (document.getElementById("big-task-pop-up-title-text")) {
    let task = getIndexOfTask(activeTaskID)
...
function renderBigTask(jsonTextElement) {
  ...
  document.getElementById("big-task-pop-up-category").innerHTML = taskJson.type;
  document.getElementById("big-task-pop-up-category").style.backgroundColor = checkCategoryColor(taskJson.type);
  ...
  document.getElementById("big-task-pop-up-bottom-buttons-container").innerHTML = returnDeleteEditHTML(taskJson.id);

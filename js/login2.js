...
async function loginFunction(event) {
    ...
    if (sessionStorage.getItem("AccessToken") != ("" | "undefined") && sessionStorage.getItem("AccessToken") === response.token) {
      if (remember) {
        localStorage.setItem("isLoggedIn", "true");
        localStorage.setItem("AccessToken", `${response['token']}`)
      }
      sessionStorage.setItem("userNickname", response.username)
  ...
  async function signUp(event) {
    ...
    let user = buildUserFunction(name, email, password, passwordRepeat);
  ...
  */
  ...
  function buildUserFunction(name, email, password, passwordRepeat) {
    let user = {
      username: name,
      email: email,
      password: password,
  ...
  async function sentToken() {
    try {
      let response = await fetch('http://127.0.0.1:8000/auth/guest-login/', {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async function postDataToDatabase(path, data) {
    const BASE_URL = "http://127.0.0.1:8000/auth/registration/"
